...
$panggilAction = function () {
    ...
    // Spell out BK-XXXXX per digit for clearer sound
    $idStr = str_pad($booking->id, 5, '0', STR_PAD_LEFT);
    $formattedCode = 'B, K, ' . implode(', ', str_split($idStr));
    
    // Replace 0 with 'kosong' for more natural sounding ID calls
    $formattedCode = str_replace('0', 'kosong', $formattedCode);

    $text = "Nomor antrian, {$formattedCode}, atas nama, {$booking->name}. Silakan menuju {$loket->name}.";
...
<div class="p-6" wire:poll.5s>
    <div 
        x-data="{
            panggilSuara(text) {
                ...
                // Coba cari suara Bahasa Indonesia yang bagus
        ...
        @play-call.window="panggilSuara($event.detail.text)"
    >
        <!-- Test Voice Button for User Activation -->
        <div class="fixed bottom-4 right-4 z-50">
            <button @click="panggilSuara('Nomor antrian, B, K, kosong, satu. Silakan.')" class="bg-indigo-600/20 backdrop-blur-md text-[10px] font-black uppercase tracking-[0.2em] px-4 py-2 rounded-full border border-indigo-500/30 hover:bg-indigo-600/40 transition-all text-indigo-400 shadow-xl">
                🔊 Tes Panggilan
            </button>
        </div>
    <x-slot name="header">
                ...
                <div class="relative z-10">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-4 bg-gray-50 dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-700">
                            <div class="text-[10px] text-gray-500 uppercase font-black tracking-widest text-center sm:text-left">Total Antrian</div>
                        ...
                        @forelse($this->queuePending as $index => $item)
                            <div @click="$wire.openModal({{ $item->id }})" class="p-4 sm:p-6 flex items-center gap-4 sm:gap-6 hover:bg-emerald-50 dark:hover:bg-emerald-900/10 transition group/item cursor-pointer">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="text-[9px] sm:text-xs font-bold text-indigo-500 uppercase tracking-widest mt-1">BK-{{ str_pad($item->id, 5, '0', STR_PAD_LEFT) }}</div>
                            ...
                            <div class="p-6 flex items-center gap-6 hover:bg-gray-50 dark:hover:bg-gray-700 transition opacity-75">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">
                                        BK-{{ str_pad($item->id, 5, '0', STR_PAD_LEFT) }} 
    ...
    @if($showModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/80 backdrop-blur-sm px-6">
            <div class="bg-white dark:bg-gray-800 rounded-[3rem] shadow-2xl w-full max-w-xl overflow-hidden border border-gray-100 dark:border-gray-700 mx-auto" x-transition>
                <div class="p-8 text-center border-b border-gray-50 dark:border-gray-700">
                    <h3 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Proses Antrian</h3>
                    <p class="text-sm text-gray-500 mt-1">Pilih loket sebelum melakukan tindakan.</p>
                </div>
                ...
                <div class="p-8 space-y-8">
                    ...
                    @if($currentBooking)
                        <div class="flex items-center gap-6 p-6 bg-indigo-50 dark:bg-indigo-900/20 rounded-[2rem] border border-indigo-100 dark:border-indigo-800/50 shadow-inner">
                            <div class="w-16 h-16 shrink-0 flex items-center justify-center bg-white dark:bg-gray-800 text-indigo-600 dark:text-indigo-400 rounded-2xl font-black text-2xl border border-indigo-100 dark:border-indigo-800 shadow-sm">
                                BK
                            </div>
                            <div class="min-w-0">
                                <div class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-1">Sedang Diproses</div>
                                <div class="text-xl font-black text-gray-900 dark:text-white uppercase truncate">{{ $currentBooking->name }}</div>
                                <div class="text-sm font-bold text-gray-500 italic mt-0.5 truncate">{{ $currentBooking->service }}</div>
                            </div>
                        </div>
                    @endif

                    <div class="space-y-4">
                        <label class="text-xs font-black text-gray-400 uppercase tracking-widest px-1">Pilih Loket Pelayanan</label>
                        <div class="grid grid-cols-2 gap-4">
                            @foreach($this->lokets as $loket)
                                ...
                                <button 
                                    ...
                                    class="p-5 rounded-[2rem] border-2 transition-all flex flex-col items-center gap-2 group relative overflow-hidden {{ $selectedLoketId == $loket->id ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/30' : ($isFull ? 'bg-rose-50 border-rose-100 cursor-not-allowed opacity-60' : 'border-gray-100 dark:border-gray-700 hover:border-indigo-300') }}"
                                >
                                    <div class="text-[10px] font-black uppercase tracking-widest {{ $selectedLoketId == $loket->id ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 group-hover:text-indigo-400' }}">Loket</div>
                                    <div class="text-xl font-black {{ $selectedLoketId == $loket->id ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-700 dark:text-gray-300' }}">{{ $loket->name }}</div>
                                    
                                    <div class="w-full h-1 bg-gray-100 dark:bg-gray-800 rounded-full mt-2 overflow-hidden shadow-inner">
                                    ...
                                    <div class="text-[9px] font-black {{ $isFull ? 'text-rose-500 animate-pulse' : 'text-gray-400' }} uppercase tracking-tighter">
                                        {{ $count }}/60 Selesai
                                    </div>

                                    @if($isFull)
                                        <div class="absolute inset-0 bg-white/40 backdrop-blur-[1px] flex items-center justify-center">
                                            <span class="bg-rose-600 text-white text-[8px] font-black px-2 py-1 rounded-full uppercase">Limit Tercapai</span>
                                        </div>
                                    @endif
                                </button>
                            @endforeach
                ...
                <div class="p-8 bg-gray-50/50 dark:bg-gray-900/30 flex gap-4">
                    <button type="button" wire:click="$set('showModal', false)" class="flex-1 py-4 text-gray-500 font-bold hover:bg-gray-100 dark:hover:bg-gray-700 rounded-[1.5rem] transition">Tutup</button>
                    
                    <div class="flex-2 flex gap-3">
                        <button 
                            wire:click="panggilAction"
                            class="px-6 py-4 bg-white dark:bg-gray-800 text-indigo-600 dark:text-indigo-400 rounded-[1.5rem] font-black uppercase tracking-widest shadow-lg border border-indigo-100 dark:border-indigo-800 hover:bg-indigo-50 active:scale-95 transition"
